package com.tiny.security.aspect;
...
public class OperationLogAspect {
    ...
    private void handleLog(JoinPoint joinPoint, OperationLog operationLogAnnotation, Exception e, Object result) {
        ...
        try {
            ...
            // 设置模块和操作信息
            logDTO.setModule(operationLogAnnotation.module());
            logDTO.setOperationType(operationLogAnnotation.type().name());
            logDTO.setDescription(operationLogAnnotation.desc());

            // 从ThreadLocal获取请求信息
            RequestInfo requestInfo = REQUEST_INFO.get();
            if (requestInfo != null) {
                logDTO.setRequestMethod(requestInfo.requestMethod);
                logDTO.setRequestUrl(requestInfo.requestUrl);
                logDTO.setIp(requestInfo.ipAddr);
                logDTO.setLocation(IpUtil.getRegion(requestInfo.ipAddr));
                logDTO.setBrowser(parseBrowser(requestInfo.userAgent));
                logDTO.setOs(parseOs(requestInfo.userAgent));
            }
            ...
            if (operationLogAnnotation.saveRequestParam()) {
                ...
                logDTO.setRequestParams(truncate(params, 2000));
            }

            // 设置响应结果
            if (operationLogAnnotation.saveResponseResult() && result != null) {
                String resultJson = JSON.toJSONString(result);
                logDTO.setResponseResult(truncate(resultJson, 2000));
            }

            // 设置操作状态
            if (e != null) {
                logDTO.setStatus(1);
                logDTO.setErrorMsg(truncate(e.getMessage(), 2000));
            } else {
                logDTO.setStatus(0);
            }
            ...
            // 调用日志记录器
            operationLogRecorder.record(logDTO);
        } catch (Exception ex) {
